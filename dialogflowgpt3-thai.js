...
app.get('/', (req,res) => {
  res.send('Welcome to Dialogflow-GPT3 Thai new Application!!');
})
...
app.post("/dialogflow", express.json(), (req, res) => {
    ...
    function welcome(agent) {
      agent.add('สวัสดี ฉันชื่อเดบร้า! ฉันคือผู้ช่วยส่วนตัวเสมือนจริงของคุณจาก Orient Telecoms วันนี้คุณเป็นอย่างไร?');
  }
  ...
  async function defaultFallback(agent) {
      ...
      let query = agent.query + ' please only respond to me in thai language';
